...
MAINTAINER João Ferreira ajoao.parana@gmail.com<> 

# based on Solution by Anastas Dancha <anapsix@random.io>
...
RUN apk add curl wget bash vim

RUN cd /tmp/ && wget -O gosu "https://github.com/tianon/gosu/releases/download/1.10/gosu-amd64" && \
    chmod +x gosu && \
    RET=./gosu > gosuout.txt && cat gosuout.txt && echo $RET && /bin/true && \
    mv gosu /usr/local/bin && \
    gosu postgres:postgres bash -c 'whoami && id'

COPY docker-entrypoint.sh /entrypoint.sh

RUN addgroup -g 2000 soma  && \
    adduser -D -u 2000 -G soma soma
